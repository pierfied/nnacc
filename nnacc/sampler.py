...
class Sampler:
    def __init__(self, lnp, x0=None, m=None, transform=None, device='cpu'):
        ...
        if self.x0 is None:
            ...
            self.x0 = x0.to(torch.float32, device=device)
        ...
        if self.m is None:
            ...
            self.m = m.to(torch.float32, device=device)
    ...
    def calc_hess_mass_mat(self, nsteps=1000, eps=1e-4, resamp_x0=True):
        ...
        for i in pbar:
            ...
            x += grad * eps
            pbar.set_description('log-prob: {}'.format(x))
        ...
        self.lnp = lambda x: self.orig_lnp(self.u @ x)
        if self.transform is None:
            self.transform = lambda x: self.u @ x
        else:
            self.orig_transform = self.transform
            self.transform = lambda x: self.orig_transform(self.u @ x)

        if resamp_x0:
            self.x0 = x + torch.randn(self.nparams, device=self.device) * torch.sqrt(s)
